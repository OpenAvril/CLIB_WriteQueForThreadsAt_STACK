...
#include <iostream>
	static std::list<class wq::ClusterAccess_Framework*>* stat_REG_ptr_ClusterAccess_array_of_ptr_DataCluster;//todo number of data clusters.
...
// public.
	 void wq::ClusterAccess::generateHandles(uint8_t threadId, std::byte MAX_NUMBER_OF_DATA_CLUSTERS, std::list<std::byte> MAX_NUMBER_OF_THREADS_FOR_ACCESS_ARRAY) {
		...
		for (uint8_t handleId = 1; handleId < static_cast<uint8_t>(MAX_NUMBER_OF_DATA_CLUSTERS); handleId++) {
			...
			ClusterAccess_Framework* tempObj = *temp;
	...
	unsigned char* wq::ClusterAccess::CLIB_ClusterAccess_App_FUNCT_get_FLAG_isPGM_INSTANTIATED(uint8_t threadId, uint8_t dataClusterId)	{
		...
		*result = true;
		if (!CLIB_ClusterAccess_stat_REG_get_array_of_isMemberFunctionINSTANTIATED(threadId)->at(0)) {
			std::cout << "flag = " << std::to_string(CLIB_ClusterAccess_stat_REG_get_array_of_isMemberFunctionINSTANTIATED(threadId)->at(0)) << std::endl;
...
// private.
	...
	bool wq::ClusterAccess::CLIB_ClusterAccess_stat_APP_FUNCT_Calc_IsAllINSTANTIATED(uint8_t threadId, uint8_t dataClusterId) {
	...
	void wq::ClusterAccess::CLIB_ClusterAccess_stat_PGM_boot3_INITIALISE_array_of_ptr_DataCluster_Framework(uint8_t threadId, std::byte MAX_NUMBER_OF_DATA_CLUSTERS) {
		stat_REG_ptr_ClusterAccess_array_of_ptr_DataCluster = new std::list<class wq::ClusterAccess_Framework*>(1);
		for (uint8_t dataClusterId = 0; dataClusterId < static_cast<uint8_t>(stat_REG_ptr_ClusterAccess_array_of_ptr_DataCluster->size()); dataClusterId++) {
			...
			*temp = static_cast<wq::ClusterAccess_Framework*>(wq::ClusterAccess::CLIB_ClusterAccess_App_FUNCT_generate_Program(threadId, dataClusterId));
		}
	}
	...
	std::list<class wq::ClusterAccess_Framework*>* wq::ClusterAccess::CLIB_ClusterAccess_stat_PGM_get_array_of_ptr_DataCluster(uint8_t threadId) {

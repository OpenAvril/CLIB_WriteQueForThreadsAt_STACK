...
namespace CLIB_ThreadLogs {
    ...
    private:
        static void* _stat_PGM_CLIB_WriteQueForThreadsAt_DataCluster;
        static void stat_PGM_boot1_DEFINE_CLIB_WriteQueForThreadsAt_DataCluster(uint8_t threadId);
        static void stat_PGM_boot3_INITIALISE_CLIB_WriteQueForThreadsAt_DataCluster(uint8_t threadId, std::byte* MAX_NUMBER_OF_DATA_CLUSTERS, std::list<std::byte>* MAX_NUMBER_OF_THREADS_FOR_ACCESS_ARRAY);
        static void stat_PGM_boot4_INSTANTIATE_CLIB_ThreadLogs_Framework_Execute(uint8_t threadId);
        static void* stat_PGM_get_ptr_CLIB_WriteQueForThreadsAt_DataCluster(uint8_t threadId);
    };
};
